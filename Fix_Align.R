...
fix_align <- function(start_pts, 
  ...
  for (i in 1:n_files) {
    ...
    trial_list <- c(1:n_trials)
    for (t in trial_list) {
      
      trial_list = trial_list[ - which(trial_list %in% exclude_trials)]
      cat(t, "\n")
...
read_asc_file <- function(asc_file_name, start_flag) {
  ...
  for (t in trial_list) {
    ...
    if(n_fix == 0) {
      fix_data_trial <- matrix(NA, nrow=1, ncol=4)
      fix_data_trial[1, 1:4] <- c(0, 0, 0, 0)
    }
    else {
      for (i in 1:n_fix) {
...
# Selecting Files and Start Points
numbers <- c(1, 22, 24)
file <- rep("DBP", length(numbers))
numbers <- sprintf("%02d", numbers)

asc_files <- paste0("wider_aois/", "wider_", file, numbers, ".asc")
...
# Excluding Problematic Trials

library(tidyverse)
trials <- read_csv("wider_aois/wider_DBP01.asc.csv")
names(trials) <- substring(names(trials), 29, 43)

exclude_trials <- c()
if (max(trials) == 67) {
  "all trials completed" 
}  else { exclude_trials <- as.vector(cbind(exclude_trials,(max(trials))))
}

# Trying to read in all CSVs

path <- "wider_aois"
all_csv <- dir(path = path, pattern = "*.csv")

tbl <- all_csv %>%
  #map(~read_csv(file.path(all_csv))) 
  #map(function(x) read_csv(file.path(all_csv, x)))
